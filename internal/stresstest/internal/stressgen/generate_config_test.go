...
func TestGenerateConfigFile(t *testing.T) {
	tests := map[string]struct {
		...
		"variable empty": {
			[]ConfigObject{
				&ConfigVariable{
					Addr: addrs.InputVariable{Name: "blorp"}.Absolute(addrs.RootModuleInstance),
				},
			},
			`
				variable "blorp" {
		...
		"output value any optional arguments": {
			[]ConfigObject{
				&ConfigOutput{
					Addr:  addrs.OutputValue{Name: "blorp"}.Absolute(addrs.RootModuleInstance),
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			got := bytes.TrimSpace(GenerateConfigFile(test.Input))
